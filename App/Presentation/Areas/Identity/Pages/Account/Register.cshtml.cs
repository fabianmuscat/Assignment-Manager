...
public class RegisterModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync(string returnUrl = null)
    {
        ...
        var firstLetterName = Input.FirstName[0].ToString().ToUpper();
        Input.FirstName = $"{firstLetterName}{Input.FirstName[1..].ToLower()}";

        var firstLetterSurname = Input.LastName[0].ToString().ToUpper();
        Input.LastName = $"{firstLetterSurname}{Input.LastName[1..].ToLower()}";

        var user = new Student
            { IDCard = Input.IdCard, FirstName = Input.FirstName, LastName = Input.LastName, Email = Input.Email };
        ...
        if (result.Succeeded)
        {
            ...
            if (string.IsNullOrEmpty(returnUrl) || !Url.IsLocalUrl(returnUrl))
                return RedirectToAction("Redirect", "Assignments");
    ...
    {
        [Display(Name = "ID Card")] public string IdCard { get; set; }
